<div>
  <div class="max-w-6xl mx-auto p-6">
    <h1 class="text-2xl font-bold mb-4">Bienvenido, {{ $alumno->name }}</h1>
    <div class="mb-6 p-4 bg-blue-50 border border-blue-200 rounded">
      ...
      @if(!$editando)
        @if($alumno->profile_photo)
          <img src="{{ asset('storage/'.$alumno->profile_photo) }}" class="w-24 h-24 rounded-full object-cover mb-3" alt="Foto de perfil">
        @else
          <div class="w-24 h-24 rounded-full bg-gray-200 flex items-center justify-center text-gray-600 mb-3">
    ...
    <h1 class="text-2xl font-bold mb-4">Perfil Profesor</h1>
        ...
        @foreach($profesores as $prof)
          <div class="p-4 border rounded-lg shadow-sm bg-white">
            <div class="flex items-center gap-3">
              ...
              @endif
              <div>
                <h2 class="text-lg font-semibold leading-tight">{{ $prof->name }}</h2>
            ...
            <div class="mt-3 text-sm text-gray-700 space-y-1">
              @if($prof->whatsapp)
                <p><strong>WhatsApp:</strong> {{ $prof->whatsapp }}</p>
              @endif
              @if($prof->comision)
              ...
              @endif
              @if(!empty($prof->telefono))
                <p><strong>Teléfono:</strong> {{ $prof->telefono }}</p>
              @endif
              @if(!empty($prof->descripcion))
              ...
              @if($profiles->isNotEmpty())
                <ul class="space-y-1">
                  @foreach($profiles as $sp)
                    @if($sp->linkedin)
                      <li><a href="{{ $sp->linkedin }}" target="_blank" rel="noopener" class="text-blue-600 underline">LinkedIn</a></li>
                    @endif
                    @if($sp->github)
                      <li><a href="{{ $sp->github }}" target="_blank" rel="noopener" class="text-blue-600 underline">GitHub</a></li>
                    @endif
                    @if($sp->gitlab)
                      <li><a href="{{ $sp->gitlab }}" target="_blank" rel="noopener" class="text-blue-600 underline">GitLab</a></li>
                    @endif
                    @if($sp->wordpress)
                      <li><a href="{{ $sp->wordpress }}" target="_blank" rel="noopener" class="text-blue-600 underline">WordPress</a></li>
                    @endif
                    @if($sp->notion)
                      <li><a href="{{ $sp->notion }}" target="_blank" rel="noopener" class="text-blue-600 underline">Notion</a></li>
                    @endif
                  @endforeach
                </ul>
              @else
