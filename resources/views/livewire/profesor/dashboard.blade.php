<div class="p-6 min-h-screen bg-gray-100">
    ...
    <div class="flex items-center justify-between mb-8">
        ...
        <div class="flex gap-2">
            <button type="button" class="bg-gray-200 px-4 py-2 rounded hover:bg-gray-300" wire:click="acercaDe">Acerca de</button>
    ...
    <div class="bg-white rounded-xl shadow p-6 flex items-center gap-6 mb-8">
        <img
            ...
            wire:click="verFotoPerfil('{{ auth()->user()->profile_photo ? asset('storage/'.auth()->user()->profile_photo) : 'https://ui-avatars.com/api/?name='.urlencode(auth()->user()->name) }}')">
        <div>
            <p class="font-bold text-xl">{{ auth()->user()->name }}</p>
            <p class="text-gray-600">{{ auth()->user()->email }}</p>
            <div class="flex gap-2 mt-2">
                <button type="button" class="bg-blue-500 text-white px-4 py-1 rounded hover:bg-blue-600" wire:click="editarPerfil">
                    Editar Perfil
                </button>
            </div>
        </div>
    ...
    <div id="lista-alumnos" class="bg-white rounded-xl shadow p-6">
        ...
        @if(session()->has('mensaje'))
            {{-- Se auto-oculta en 3s usando Livewire (sin JS manual) --}}
        ...
        <div class="mb-4 relative">
            <input
                type="text"
                wire:model.debounce.200ms="q"
                wire:keydown.enter="buscarAhora"
                ...
                <tbody>
                    @foreach($alumnos as $alumno)
                        <tr class="border-b hover:bg-gray-50">
                            <td class="p-3">
                                <img
                                    src="{{ $alumno->profile_photo ? asset('storage/'.$alumno->profile_photo) : 'https://ui-avatars.com/api/?name='.urlencode($alumno->name) }}"
                                    class="w-12 h-12 rounded-full object-cover border-2 border-green-400 cursor-pointer"
                                    wire:click="verFotoPerfil('{{ $alumno->profile_photo ? asset('storage/'.$alumno->profile_photo) : 'https://ui-avatars.com/api/?name='.urlencode($alumno->name) }}')">
                            </td>

                            <td class="p-3 font-medium">{{ $alumno->name }}</td>
                            <td class="p-3">{{ $alumno->email }}</td>

                            {{-- Redes (WhatsApp + GitHub + GitLab + Notion + WordPress) --}}
                            <td class="p-3">
                                <div class="flex items-center gap-2">
                                    @if($alumno->whatsapp)
                                        <a href="https://wa.me/{{ $alumno->whatsapp }}" target="_blank" title="WhatsApp" class="hover:opacity-80" rel="noopener">
                                    ...
                                    @if($alumno->github)
                                        <a href="{{ $alumno->github }}" target="_blank" title="GitHub" class="hover:opacity-80" rel="noopener">
                                    ...
                                    @if($alumno->gitlab)
                                        <a href="{{ $alumno->gitlab }}" target="_blank" title="GitLab" class="hover:opacity-80" rel="noopener">
                                            <svg class="w-6 h-6" viewBox="0 0 24 24" fill="currentColor">
                                    ...
                                    @if($alumno->notion)
                                        <a href="{{ $alumno->notion }}" target="_blank" title="Notion" class="hover:opacity-80" rel="noopener">
                                            <svg class="w-6 h-6" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                                                <rect x="3" y="3" width="18" height="18" rx="3"></rect>
                                                <path d="M8 8h2l4 6V8h2v8h-2l-4-6v6H8V8z" fill="white"></path>
                                            </svg>
                                        </a>
                                    @endif

                                    @if($alumno->wordpress)
                                        <a href="{{ $alumno->wordpress }}" target="_blank" title="WordPress" class="hover:opacity-80" rel="noopener">
                                            <svg class="w-6 h-6" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                                                <circle cx="12" cy="12" r="10"></circle>
                                                <path d="M6.5 8.5c.8 0 1.3.5 1.5 1.1l2.7 7.7L8.5 12c-.3-.7-.6-1.4-.6-2 0-.6.2-1 .6-1.5H6.5zM12 7c.8 0 1.5.2 2 .5-.5.5-.8 1.2-.8 2 0 .8.4 1.8.8 3l1.6 4.5c-1 .5-2 .8-3 .8-1.1 0-2.1-.3-3-.8L12 7zM15.6 8.5h1.9c.3.5.5 1 .5 1.6 0 .8-.4 1.8-.8 3l-1.2 3.3-2.1-6c-.2-.6-.4-1.1-.4-1.6 0-.6.2-1.1.6-1.6.5-.3 1-.5 1.6-.5z" fill="white"></path>
                                            </svg>
                                        </a>
                                    @endif
                                </div>
                            ...
                            <td class="p-3">
                                <div class="flex items-center gap-2">
                                    @if($alumnoSeleccionado && $alumnoSeleccionado->id === $alumno->id)
                                        <button type="button"
                                            wire:click="toggleVerAlumno({{ $alumno->id }})"
                                            class="px-3 py-1 bg-gray-500 text-white rounded hover:bg-gray-600">
                                            Ocultar
                                        </button>
                                    @else
                                        {{-- Anchor a #detalle-alumno para bajar sin JS + acción Livewire --}}
                                        <a href="#detalle-alumno"
                                           wire:click="toggleVerAlumno({{ $alumno->id }})"
                                           class="px-3 py-1 bg-blue-500 text-white rounded hover:bg-blue-600 inline-block">
                                           Ver
                                        </a>
                                    @endif
    ...
    @if($alumnoSeleccionado)
        <div id="detalle-alumno" class="mt-8 p-6 bg-white rounded-xl shadow flex flex-col md:flex-row gap-6 items-center">
            <img
                src="{{ $alumnoSeleccionado->profile_photo ? asset('storage/'.$alumnoSeleccionado->profile_photo) : 'https://ui-avatars.com/api/?name='.urlencode($alumnoSeleccionado->name) }}"
                class="w-32 h-32 rounded-full object-cover border-2 border-green-400 cursor-pointer"
                wire:click="verFotoPerfil('{{ $alumnoSeleccionado->profile_photo ? asset('storage/'.$alumnoSeleccionado->profile_photo) : 'https://ui-avatars.com/api/?name='.urlencode($alumnoSeleccionado->name) }}')">
            <div>
        ...
        <div class="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
            <div class="bg-white p-6 rounded-xl shadow-lg w-full max-w-md">
                ...
                <div class="flex gap-2 mt-4 justify-end">
                    ...
                    <button type="button" wire:click="eliminarAlumno({{ $confirmarEliminarId }})"
                            class="bg-red-500 text-white px-4 py-2 rounded hover:bg-red-600">Sí, eliminar</button>
                </div>
            </div>
        </div>
    @endif
